...
@Component({
    selector: 'dialog-form-sales_dp',
    templateUrl: './form.html',
    styleUrls: ['../sales-inv-dp.component.scss'],
    encapsulation: ViewEncapsulation.None
})
export class SalesDpDialogFormComponent implements OnInit {
    ...
    constructor(
        private core: Core,
        public dialogRef: MatDialogRef<SalesDpDialogFormComponent>,
    ...
    ngOnInit() {
        ...
        if (this.form.id != 'add') {
            this.form.dp_amount = this.form.sold_price / 100 * this.form.dp;
            let List = {
                item    : this.form.item,
            ...
            this.form.dp_amount = Subtotal / 100 * this.form.dp;
    ...
    SCCodeSelect(e, item) {
        ...
        if (e.isUserInput) {
            ...
            this.form.dp_amount = item.sold_price / 100 * item.dp;
            let List = {
                item    : item.item,
                nama    : item.item_nama,
                qty     : item.qty,
                price   : item.sold_price,
                satuan  : item.item_satuan
            };
            this.List.push(List);
            this.form.list = this.List;
