...
class DB {
    ...
    public function ManualCommit($manual = true){
    ...
    /** BEGIN DBInsert */
    public function Insert($Table, $Fields = array(), $Params = array()){
        ...
        if($this->table_exist($Table, DBDEF)){
            ...
            //     $this->conn->error
            // );
            $return = $this->conn->query($Query)  or $this->LogError(
                $Query . "\n" . 
                "================\n" . 
                "Table " . DBDEF . "." . $Table . "\n" . 
                "================\n" . 
            ...
            if(
                ...
                $this->conn->query("
                    ...
                    VALUES(" . $this->DBJoinArray($TOCL) . ")
                ") or $this->LogError ('INSERT CLONE FAILED -> ' . $this->conn->error);

                /**
                 * Weighbridge Clone
                 */
                if($Params['wbclone'] == true){
                    $WBCL = array(
                        DBDEF,
                        $Table,
                        $Query,
                    ...
                    //         `time`
                    //     )
                    //     VALUES(" . $this->DBJoinArray($WBCL) . ")
                    // ") or die ('WB INSERT CLONE FAILED -> ' . $this->conn->error);
                    $this->conn->query("
                        INSERT INTO
                            wb_clone
                        (
                        ...
                        VALUES(" . $this->DBJoinArray($WBCL) . ")
                    ") or $this->LogError ('WB INSERT CLONE FAILED -> ' . $this->conn->error);
                }
                //=> / END : Weighbridge Clone
    ...
    /** BEGIN Update */
    public function Update($Table, $Fields = array(), $Where, $Params = array()){
        if(empty($Where)){
            ...
            $TLogReturn = $this->conn->query($TLogQuery) or $this->LogError(
                $TLogQuery . "\n" . 
                "================\n" . 
                "Table " . $Table . "\n" . 
                "================\n" .
            ...
            $TLogrows = array();
            while($r = $this->Result($TLogReturn)) {
                $TLogrows[] = $r;
            }
            ...
            $this->conn->query("
                INSERT INTO
                    _tlog
                (
                    `table`, `data_id`, `data_json`, `query`, `action`, `action_by`, `time`, `status`
                )
                VALUES(" . $this->DBJoinArray($TLOG) . ")
            ") or $this->LogError ('UPDATE TLOG FAILED -> ' . $this->conn->error);
             ...
             * Clone
             */
            if(DEF['clone'] && $return && $Table != $TableTocl){
                ...
                $this->conn->query("
                    ...
                    VALUES(" . $this->DBJoinArray($TOCL) . ")
                ") or $this->LogError ('UPDATE CLONE FAILED -> ' . $this->conn->error);
                ...
                if($Params['wbclone'] == true){
                    ...
                    //     )
                    //     VALUES(" . $this->DBJoinArray($WBCL) . ")
                    // ") or die ('WB UPDATE CLONE FAILED -> ' . $this->conn->error);
                    $this->conn->query("
                        INSERT INTO
                            wb_clone
                        (
                        ...
                        VALUES(" . $this->DBJoinArray($WBCL) . ")
                    ") or $this->LogError ('WB UPDATE CLONE FAILED -> ' . $this->conn->error);
                }
                //=> / END : Weighbridge Clone
        ...
        if($this->table_exist($Table, DBDEF)){
            ...
            if(DEF['clone'] && $return && $Table != $TableTocl){
                ...
                $this->conn->query("
                    ...
                    VALUES(" . $this->DBJoinArray($TOCL) . ")
                ") or $this->LogError ('DELETE CLONE FAILED -> ' . $this->conn->error);
                ...
                if($Params['wbclone'] == true){
                    ...
                    //     )
                    //     VALUES(" . $this->DBJoinArray($WBCL) . ")
                    // ") or die ('WB DELETE CLONE FAILED -> ' . $this->conn->error);
                    $this->conn->query("
                        INSERT INTO
                            wb_clone
                        (
                        ...
                        VALUES(" . $this->DBJoinArray($WBCL) . ")
                    ") or $this->LogError ('WB DELETE CLONE FAILED -> ' . $this->conn->error);
                }
                //=> / END : Weighbridge Clone
    ...
    public function LogError($MSG){
        ...
        echo Core::ReturnData(array(
            'critical' => 1,
            'status' => 0,
            'error_msg' => CLEANHTML($MSG)
        ));
        exit();
    ...
    public function QueryPort($Query, $DBName = ""){
        ...
        if(!empty($Query)){
            ...
            if($this->conn->select_db($DBName)){
                ...
                //     $this->conn->error
                // );
                $Result = $this->conn->query($Query) or $this->LogError(
                    $Query . "\n" . 
                    "================\n" . 
                    "Database " . $DBName . "\n" . 
                    "================\n" . 
    ...
    public function QueryMulti($Query, $DBName = ""){
        ...
        if(!empty($Query)){
            ...
            if($this->conn->select_db($DBName)){
                ...
                //     $this->conn->error
                // );
                $Result = $this->conn->multi_query($Query) or $this->LogError(
                    $Query . "\n" . 
                    "================\n" . 
                    "Database " . $DBName . "\n" . 
                    "================\n" . 
    ...
    public function Count(
        ...
        if(empty($QSql)){
            ...
            //     $this->conn->error
            // );
            $Query = $this->conn->query($Query) or $this->LogError(
                $Query . "\n" . 
                "================\n" . 
                "Table " . $DBName . "." . $Table . "\n" . 
                "================\n" . 
