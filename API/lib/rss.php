...
class rss_feed  {
  ...
  public function get_feed_items($rss_date, $rss_items_count = 10) {
    ...
    foreach($a_topic_ids as $topic_id) {
      ...
      $sql='SELECT * FROM artikel WHERE id=' . $topic_id;
      $rs=$conn->query($sql);
      ...
      // title
      $a_rss_item['title'] = htmlspecialchars(htmlentities($topic['judul']));
 
      // link
      $a_rss_item['link'] = HOST . '/baca/' . $topic['id'] . '/' . title($topic['judul']);
 
      // description
	  $DESC = $topic['intro'];
	  /*if(empty($topic['intro'])){
		$DESC = substr(htmlspecialchars(htmlentities(CLEANHTML($topic['content']))), 0, 100);
	  }*/
	  
	  $CONTENT = explode("<p>", $topic['content']);
	  $TRACE = '';;
	  //=> Trace Content
	  for($i = 0; $i < sizeof($CONTENT); $i++){
		  $CHECK = htmlspecialchars(htmlentities(CLEANHTML(strip_tags($CONTENT[$i]))));
		  if(!empty($CHECK)){
			  $TRACE = $CHECK;
			  break;
		  }
      ...
      $a_rss_item['description'] = $DESC;
 
      /*if($topic['image']) {
        $img_url = HOST . '/baca/' . $topic['id'] . '/' . title($topic['judul']);
        $a_rss_item['description'] = '<img src="' . $img_url . '" hspace="5" vspace="5" align="left"/>';
      }*/
	  /*if(!empty($topic['cover'])){
		$COVER = HOST . "/images/upload/_thumbnail/" .  $topic['cover'];
		$a_rss_item['description'] = '<img src="' . $COVER . '" hspace="5" vspace="5" align="left"/>';
	  }
      $a_rss_item['description'] .= $DESC;*/
 
      // pubdate -> configure appropriately to your environment
      $date = new DateTime($topic["date_time"]);
